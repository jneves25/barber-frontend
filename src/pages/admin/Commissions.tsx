...
const AdminCommissions = () => {
	const { companySelected, hasPermission, user } = useAuth();
	...
	useEffect(() => {
		if (!isAdmin) {
			toast.error('Você não tem permissão para acessar a área de comissões. Apenas administradores podem gerenciar comissões.');
			navigate('/admin');
		}
	}, [isAdmin, navigate]);
	...
	const handleCommissionTypeChange = async (barberId: number, type: CommissionTypeEnum) => {
		try {
			...
			if (type === CommissionTypeEnum.GENERAL) {
				toast.success("Alterado para comissão geral. Configure a porcentagem desejada.");
				handleOpenSettings(barberId);
			} else {
				toast.success("Alterado para comissão por serviço. Configure as porcentagens para cada serviço.");
				handleOpenServiceForm(barberId);
			}
		} catch (error) {
			toast.error("Erro ao alterar tipo de comissão");
		}
	};
	...
	const updateServiceCommission = async (barberId: number, serviceId: number, value: number, serviceType: CommissionRuleTypeEnum) => {
		try {
			...
			if (!existingRule) {
				toast.error("Regra de comissão não encontrada para este serviço");
				return;
			}
			...
			toast.error("Erro ao atualizar comissão do serviço");
		}
	};
	...
	const updateBarberGeneralCommission = async (barberId: number, value: number) => {
		try {
			...
			toast.success(`Comissão geral atualizada para ${value}%`);
			await fetchCommissionData();
		} catch (error) {
			toast.error("Erro ao atualizar comissão geral");
		}
	};
