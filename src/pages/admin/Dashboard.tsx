  ...
  return (
    <Card
      className={`mb-3 border-l-4 ${
        isFirst ? "border-l-barber-600" : "border-l-gray-300"
        ...
        <div className="mr-4 mt-1">
          <div
            className={`p-2 rounded-full ${
              isFirst ? "bg-barber-100" : "bg-gray-100"
            }`}
          >
            {isFirst ? (
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-1">
            ...
            <div>
              ...
              {isTomorrow && (
                <Badge className="bg-blue-500 hover:bg-blue-600">Amanhã</Badge>
              )}
              {!isToday && !isTomorrow && (
          ...
          <div className="flex items-center justify-between text-xs">
            <span className="font-medium text-barber-600">
              {format(appointmentDate, "HH:mm")}
            </span>
            <span className="font-medium">{`R$ ${appointment.value
              .toFixed(2)
              .replace(".", ",")}`}</span>
          </div>
        </div>
...
const Dashboard = () => {
  ...
  const [currentMonthPendingCommission, setCurrentMonthPendingCommission] =
    useState(0);
  const [comissoesProjetadasValor, setComissoesProjetadasValor] =
    useState("R$ 0,00");
    ...
    const loadDashboardData = async () => {
      try {
        ...
        const startDate = format(
          dateRange[0] || startOfDay(new Date()),
          "yyyy-MM-dd"
        );
        const endDate = format(
          dateRange[1] || endOfDay(new Date()),
          "yyyy-MM-dd"
        );
        ...
        const barberCommissionsResponse =
          await statisticsService.getBarberCommissions(
            companySelected.id,
            "custom",
            startDateFormatted,
            endDateFormatted
        ...
        if (
          barberCommissionsResponse.success &&
          barberCommissionsResponse.data
        ) {
          ...
          if (isAdmin) {
            ...
            const totalAppointments = barberCommissionsResponse.data.reduce(
              (sum, barber) => sum + (barber.appointmentCount || 0),
              0
            );
            ...
            try {
              const allAppointmentsResponse = await AppointmentService.getAll(
                companySelected.id
              );

              if (
                allAppointmentsResponse?.success &&
                ...
                const periodPendingValue = pendingApps.reduce(
                ...
                setProjectionData({
                  ...
                  totalProjectedCommission: pendingApps.reduce(
                    (sum, app) => sum + app.value * 0.2,
                    0
                  ), // 20% de comissão
        ...
        if (!isAdmin && user?.id) {
          ...
          const goalsResponse = await goalService.getUserGoals(
            currentMonth,
            currentYear
          );
  ...
  useEffect(() => {
    ...
    const loadProjections = async () => {
      try {
        const projectionsResponse = await statisticsService.getProjections(
          companySelected.id
        );
        ...
        if (projectionsResponse.success && projectionsResponse.data) {
          ...
          if (!isAdmin && user?.id) {
            ...
            const userProjection =
              projectionsResponse.data.barberProjections.find(
                (barber) => barber.id === user.id
              );

            if (userProjection) {
              console.log(
                "Projeção de comissão carregada da API:",
                userProjection
              );
              ...
              const pendingApps = await AppointmentService.getByBarber(
                companySelected.id
              );
    ...
    const loadUserGoal = async () => {
      try {
        ...
        if (response.success && response.data && response.data.length > 0) {
          ...
          const userGoal = response.data.find(
  ...
  useEffect(() => {
    const calcularComissoesProjetadas = async () => {
      try {
        ...
        const agendamentosDoPeriodo = periodPendingAppointments.filter(
          (app) => {
        ...
        const comissoesProjetadas = await agendamentosDoPeriodo.reduce(
          async (totalPromise, agendamento) => {
            const total = await totalPromise;

            // Buscar configuração de comissão do barbeiro
            const commissionConfig =
              await statisticsService.getBarberCommissionConfig(
                companySelected.id,
                agendamento.user?.id || 0
              );

            // Calcula comissão para cada serviço do agendamento
            const comissaoAgendamento = await agendamento.services.reduce(
              async (sumPromise, servico) => {
                const sum = await sumPromise;
                const valorServico =
                  servico.service.price * (servico.quantity || 1);
                ...
                if (commissionConfig?.data) {
                  if (commissionConfig.data.commissionType === "SERVICES") {
                    ...
                    commissionConfig.data.commissionType === "GENERAL"
                  ) {
                    percentualComissao = commissionConfig.data.commissionValue;
                  }
                }

                const comissaoServico =
                  (valorServico * percentualComissao) / 100;
                ...
                console.log(`[Dashboard] Comissão calculada:`, {
                  ...
                  tipoComissao:
                    commissionConfig?.data?.commissionType || "PADRAO",
                });
  ...
  const formatDateRange = () => {
    ...
    return `${format(dateRange[0], "dd/MM/yyyy")} - ${format(
      dateRange[1],
      "dd/MM/yyyy"
    )}`;
  };
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">
                {dateRange[0] && dateRange[1]
                  ? `Dados de ${format(dateRange[0], "dd/MM/yyyy")} a ${format(
                      dateRange[1],
                      "dd/MM/yyyy"
                    )}`
            ...
            >
                    ...
                    <div className="space-y-6 mb-8">
                      <Card className="shadow-md hover:shadow-lg transition-shadow">
                        <CardHeader>
                          ...
                          <CardDescription>
                            Previsão completa de receita do período selecionado
                          </CardDescription>
                        </CardHeader>
                        <CardContent>
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                            ...
                            <div className="bg-purple-50 p-4 rounded-lg">
                              ...
                              <p className="text-xs text-gray-500 mt-1">
                                Faturamento Previsto - Comissões Projetadas
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="mb-6 p-4 bg-gradient-to-r from-gray-50 to-indigo-50 rounded-lg border border-indigo-100">
                            <h3 className="text-sm font-medium text-gray-700 mb-3">
                              Resumo da Projeção Completa do Período
                            </h3>
                              ...
                              <div className="w-full h-4 bg-gray-200 rounded-full overflow-hidden">
                                <div className="h-full flex">
                                  <div
                                    ...
                                    title={`Finalizada: ${formatCurrency(
                                      stats.revenue.total
                                    )}`}
                                  ></div>
                                  <div
                    ...
                    <div className="grid grid-cols-1 mb-8">
                      <Card className="shadow-md hover:shadow-lg transition-shadow">
                        <CardHeader>
                          ...
                          <CardDescription>
                            Total de comissões a receber por profissional no
                            período
                          </CardDescription>
                        </CardHeader>
                        <CardContent>
                              ...
                              barberCommissions.map((barber) => (
                                <div key={barber.id} className="space-y-2">
                                  <div className="flex items-center">
                                    ...
                                    <span className="ml-auto font-medium text-barber-700">
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 mb-8">
                      ...
                      <Card className="shadow-md hover:shadow-lg transition-shadow">
                        <CardHeader>
                          <CardTitle className="text-lg text-gray-800">
                            Meus Serviços Mais Realizados
                          </CardTitle>
                          <CardDescription>
                            Distribuição por quantidade de serviços
                          </CardDescription>
                        </CardHeader>
                        <CardContent>
                          <div className="h-72">
                    ...
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 mb-8">
                      <Card className="lg:col-span-3 shadow-md hover:shadow-lg transition-shadow">
                        <CardHeader>
                          ...
                          <CardDescription>
                            Resumo dos ganhos atuais e futuros
                          </CardDescription>
                        </CardHeader>
                        <CardContent>
                              ...
                              <div className="mt-2">
                                {pendingAppointments.length > 0 && (
                                  <span className="text-xs text-gray-600">
                                    Próximo:{" "}
                                    {pendingAppointments[0]?.client?.name} -{" "}
                                    {format(
                                      new Date(
                                        pendingAppointments[0]?.scheduledTime
                                      ),
                                      "dd/MM/yyyy HH:mm"
                                    )}
                                  </span>
                            ...
                            <div className="bg-blue-50 rounded-lg p-4">
                              ...
                              <div className="flex items-center mt-2">
                                <span className="text-xs font-medium text-green-600">
                                  Finalizadas:{" "}
                                  {formatCurrency(userStats.commission.total)}
                                </span>
                          ...
                          <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <span className="font-medium text-gray-800">
                                {formatCurrency(
                                  userStats.revenue.total +
                                    pendingAppointments.reduce(
                                      (sum, app) => sum + app.value,
                                      0
                                    )
                                )}
                            ...
                            <div className="flex items-center justify-between text-xs text-gray-600">
                              <span>
                                Atual: {formatCurrency(userStats.revenue.total)}
                              </span>
                              <span>
                                Pendente:{" "}
                                {formatCurrency(
              ...
              <TabsContent value="appointments">
                <Card className="shadow-md hover:shadow-lg transition-shadow">
                  ...
                  <CardContent>{renderAppointments()}</CardContent>
                </Card>
              </TabsContent>
              ...
              <TabsContent value="goals">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                  <Card className="shadow-md hover:shadow-lg transition-shadow">
                    <CardHeader>
                      ...
                      <CardDescription>
                        Progresso em relação à meta de faturamento do mês atual
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      ...
                      <div className="mt-6 space-y-3">
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="font-medium text-gray-800">
                            {formatCurrency(userStats.revenue.total)}
                          </span>
                        </div>
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-gray-600">
                        ...
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-gray-600">
                            Pendente para alcançar a meta:
                          </span>
                          <span className="font-medium text-gray-800">
                            {formatCurrency(
                              Math.max(monthlyGoal - userStats.revenue.total, 0)
                            )}
                          </span>
                        </div>
                      </div>
                  ...
                  <Card className="shadow-md hover:shadow-lg transition-shadow">
                    <CardHeader>
                      ...
                      <CardDescription>
                        Comissões atuais e projetadas do mês atual
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="p-4 bg-blue-50 rounded-lg">
                          ...
                          <p className="text-2xl font-bold text-blue-600">
                            {formatCurrency(
                              userStats.commission.total + projectedCommission
                            )}
                          </p>
                          <p className="text-sm text-gray-600 mt-1">
                            Finalizadas + Pendentes
                ...
                <Card className="shadow-md hover:shadow-lg transition-shadow">
                  <CardHeader>
                    <CardTitle className="text-lg text-gray-800">
                      Detalhes dos Agendamentos Pendentes
                    </CardTitle>
                    <CardDescription>
                      Agendamentos que ainda serão realizados
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    {pendingAppointments.length > 0 ? (
                        ...
                        {pendingAppointments.slice(0, 5).map((appointment) => (
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between text-sm text-gray-600">
                              ...
                              <span>
                                {appointment.services.length} serviço
                        ...
                        {pendingAppointments.length > 5 && (
                          <div className="text-center mt-2">
                            <p className="text-sm text-gray-600">
                              +{pendingAppointments.length - 5} outros
                              agendamentos pendentes
                            </p>
                          </div>
                        )}
          ...
          <div className="col-span-12 lg:col-span-4">
            <Card className="shadow-md overflow-hidden h-full">
              ...
              <CardContent className="p-4">
                <div className="py-2">{renderAppointments()}</div>
