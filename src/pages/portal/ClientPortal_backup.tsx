...
import { Scissors, Calendar, Clock, Star, MapPin, Phone, Loader2, CalendarDays, Mail, ChevronRight, ChevronLeft, InfoIcon } from 'lucide-react';
...
import AppointmentService, { Appointment, AppointmentStatusEnum, Service as AppointmentServiceType } from '@/services/api/AppointmentService';
...
const ClientPortal = () => {
	...
	const updateVisibleDates = (page: number, allOptions?: DateOption[]) => {
		...
		if (startIndex >= optionsToUse.length) {
			setCurrentDatePage(0);
			setDateOptions(optionsToUse.slice(0, datesPerPage));
		} else {
			setCurrentDatePage(page);
			setDateOptions(optionsToUse.slice(startIndex, startIndex + datesPerPage));
		}
	};
	...
	const getCurrentMonthLabel = () => {
		...
		return format(firstDate, 'MMMM yyyy', { locale: ptBR });
	};
	...
	const handleSubmit = async (data: any) => {
		...
		if (selectedServicesObj.length === 0) {
			toast.error("Serviços não encontrados");
			setIsSubmitting(false);
			return;
		}

		try {
			// Construir a data e hora do agendamento
			const [hours, minutes] = selectedTime.split(':').map(Number);
			const scheduledDate = new Date(selectedDateOption.date);
			scheduledDate.setHours(hours, minutes, 0, 0);

			// Montar o array de serviços para o agendamento
			const servicesArray = selectedServicesObj.map(serviceObj => {
				// Converter para o formato esperado pela API
				const serviceForAppointment: AppointmentServiceType = {
					id: serviceObj.id,
			...
			const appointmentData = {
				clientId: 1, // Assumindo que o clientId está fixo por enquanto
				...
				scheduledTime: scheduledDate.toISOString(), // Converter diretamente para string
			...
			if (response.success) {
				// Salvar os dados do cliente no localStorage
				try {
					const clientData = {
						name: data.name,
						phone: data.phone,
			...
			<main className="container mx-auto px-4 py-8">
				<div className="max-w-3xl mx-auto">
					<div className="bg-white rounded-xl shadow-sm mb-6 overflow-hidden">
						...
						{!phoneVerified ? (
							<div className="p-6 border-b border-gray-100">
								<h2 className="text-lg font-semibold mb-4">Primeiro, vamos verificar seu telefone</h2>
									...
									{showNameInput && !isExistingClient && (
										<div className="space-y-4">
											...
											<Button
												...
												disabled={!customerName || customerName.length < 3}
												className="w-full">
												Continuar
											</Button>
										</div>
															...
															<div className="flex-1">
																				...
																				<div>
																					...
																					<div className="grid grid-cols-7 gap-2">
																						{dateOptions.map((option, index) => (
																							<div
																								key={index}
																								className={`border rounded-lg p-3 text-center cursor-pointer transition-all duration-200 transform hover:scale-[1.01] ${selectedDateOption?.date.toISOString() === option.date.toISOString()
																									? 'bg-barber-500 text-white ring-2 ring-barber-300'
																									: 'hover:border-barber-300 bg-white'
																									}`}
																								onClick={() => setSelectedDateOption({ label: format(option.date, 'dd/MM/yyyy', { locale: ptBR }), value: option.date.toISOString(), date: option.date })}
																							>
																								<div className="flex flex-col items-center">
																									<span className="text-sm font-medium">{format(option.date, 'dd', { locale: ptBR })}</span>
																									{format(option.date, 'MMM', { locale: ptBR }) !== 'Jan' && format(option.date, 'MMM', { locale: ptBR }) !== 'Feb' && (
																					...
																					{hasSavedClientData && (
																						<div className="bg-green-50 p-3 rounded-lg border border-green-200 mb-4">
																							<div className="flex items-center text-green-700">
																								...
																								<p className="text-sm font-medium">Seus dados foram carregados de um agendamento anterior</p>
																							</div>
																						</div>
																					)}
																					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
																						<div>
																							...
																							<Input {...form.register('name', { required: true })} placeholder="Seu nome" className="bg-white" />
																						</div>
																						<div>
																							<label className="block text-sm font-medium mb-2">Telefone</label>
																							<Input {...form.register('phone', { required: true })} placeholder="(99) 99999-9999" className="bg-white" />
																						</div>
																					</div>

																					<div>
																						<label className="block text-sm font-medium mb-2">E-mail</label>
																						<Input {...form.register('email', { required: true })} type="email" placeholder="seu@email.com" className="bg-white" />
																					</div>

																					<div>
																						<label className="block text-sm font-medium mb-2">Observações (opcional)</label>
																						<Input {...form.register('notes')} placeholder="Alguma observação para o atendimento?" className="bg-white" />
																					</div>
																				</div>
																				...
																				<div className="mt-6 flex justify-between">
																					...
																					<TooltipProvider>
																						<Tooltip>
																							<TooltipTrigger asChild>
																								<span>
																									<Button
																										type="submit"
																										className="bg-barber-500 hover:bg-barber-600"
																										disabled={isSubmitting || !form.watch('name') || !form.watch('phone') || !form.watch('email')}
																									>
																										{isSubmitting ? (
																											<>
																												<Loader2 className="mr-2 h-4 w-4 animate-spin" />
																							...
																							{(!form.watch('name') || !form.watch('phone') || !form.watch('email')) && !isSubmitting && (
																								<TooltipContent className="bg-red-50 border border-red-200 text-red-700 p-2">
																									<p>
																										{!form.watch('name')
																											? "Preencha seu nome"
																...
																<div className="bg-white rounded-xl shadow-sm p-5">
																	<div className="grid grid-cols-1 md:grid-cols-2 gap-5">
																		<div>
																			<h2 className="text-lg font-medium mb-3">Sobre {company.name}</h2>
																			<div className="flex items-center text-gray-600 mb-2">
																				<MapPin className="h-4 w-4 mr-2 text-barber-500" />
																				<span className="text-sm">{company.address}</span>
																			</div>
																			{company.phone && (
																				<div className="flex items-center text-gray-600 mb-2">
																					<Phone className="h-4 w-4 mr-2 text-barber-500" />
																					<span className="text-sm">{company.phone}</span>
																				</div>
																			)}
																			{company.email && (
																				<div className="flex items-center text-gray-600">
																					<Mail className="h-4 w-4 mr-2 text-barber-500" />
																					<span className="text-sm">{company.email}</span>
																				</div>
																			)}
																		</div>

																		<div className="flex flex-col items-center justify-center md:items-end text-center md:text-right">
																			...
																			<p className="text-sm text-gray-600">5.0 (48 avaliações)</p>
																		</div>
																	</div>
																</div>
															</div>
